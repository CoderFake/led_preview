...
from config import (
    DEFAULT_FPS, DEFAULT_LED_COUNT, IN_PORT, MOBILE_APP_OSC_PORT, MOBILE_APP_OSC_IP, DEFAULT_OSC_IP,
    DEFAULT_TRANSPARENCY, DEFAULT_LENGTH, DEFAULT_MOVE_SPEED,
    DEFAULT_MOVE_RANGE, DEFAULT_IS_EDGE_REFLECT,
    DEFAULT_DIMMER_TIME, DEFAULT_DIMMER_TIME_RATIO, update_mobile_config
)
...
def create_default_segments(effect: LightEffect, count: int = 3):
    ...
    for i in range(1, count + 1):
        segment = LightSegment(
            ...
            length=[(INITIAL_POSITION[i+1]-INITIAL_POSITION[i]) if i<10 else (DEFAULT_LED_COUNT-INITIAL_POSITION[i]), 0, 0],
            move_speed=DEFAULT_MOVE_SPEED * (1 if i % 2 == 0 else -1),
            move_range=DEFAULT_MOVE_RANGE,
            initial_position=INITIAL_POSITION[i],                    # center_position - 30 + i * 30,
...
def create_default_effects(scene: LightScene, num_effects: int = 3):
    for effect_id in range(1, num_effects + 1): 
        ...
        create_default_segments(effect, count=10)
...
def parse_arguments():
    ...
    parser.add_argument('--mobile-port', type=int, default=MOBILE_APP_OSC_PORT, help=f'Mobile app OSC port (default: {MOBILE_APP_OSC_PORT})')
    parser.add_argument('--mobile-ip', type=str, default=MOBILE_APP_OSC_IP, help=f'Mobile app OSC IP (default: {MOBILE_APP_OSC_IP})')
    parser.add_argument('--no-gui', action='store_true', help='Run without GUI (default: False)')
...
def main():
    ...
    # Update mobile config if mobile IP is provided
    if args.mobile_ip != MOBILE_APP_OSC_IP:
        from config import update_mobile_config
        update_mobile_config(ip=args.mobile_ip, port=args.mobile_port)
    
    logger.info(f"FPS: {args.fps}, LED Count: {args.led_count}, OSC listening on: {DEFAULT_OSC_IP}:{args.in_port}, Mobile: {MOBILE_APP_OSC_IP}:{args.mobile_port}")
    
    from config import initialize_palette_cache
    ...
    if not args.simulator_only:
        osc_handler = OSCHandler(light_scenes, ip=DEFAULT_OSC_IP, in_port=args.in_port)
