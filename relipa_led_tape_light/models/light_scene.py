...
from config import DEFAULT_COLOR_PALETTES, update_palette_cache
...
class LightScene:
    ...
    def update_palette(self, palette_id: str, colors: List[List[int]]):
        ...
        import copy
        if palette_id in self.palettes:
            self.palettes[palette_id] = copy.deepcopy(colors)
    ...
    def load_from_json(cls, file_path: str):
        ...
        if "palettes" in data:
            ...
            for palette_id, colors in data["palettes"].items():
    ...
    def load_palettes_from_json(self, file_path: str):
        ...
        self.set_palette(current_palette)

    def load_effects_from_json(self, file_path: str):
        """
        ...
        if "current_effect_ID" in data and data["current_effect_ID"] is not None:
            self.current_effect_ID = data["current_effect_ID"]
        elif self.effects:
            self.current_effect_ID = next(iter(self.effects.keys()))
        else:
            self.current_effect_ID = None