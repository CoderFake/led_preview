...
class LightSegment:
    ...
    def update_position(self, fps: int):
        ...
        if self.is_edge_reflect:
            if new_position < self.move_range[0]:
                excess = self.move_range[0] - new_position
                new_position = self.move_range[0] + excess
                self.direction = 1 
                self.move_speed = abs(self.move_speed)  
                
            elif new_position + total_length - 1 > self.move_range[1]:
                excess = new_position + total_length - 1 - self.move_range[1]
                new_position = self.move_range[1] - total_length + 1 - excess
                self.direction = -1
                self.move_speed = -abs(self.move_speed)
                
            new_position = max(self.move_range[0], min(new_position, self.move_range[1] - total_length + 1))
        else:
    ...
    def calculate_rgb(self, palette_name: str = "A") -> List[List[int]]:
        ...
        if hasattr(self, 'scene') and hasattr(self.scene, 'palettes') and palette_name in self.scene.palettes:
            palette = copy.deepcopy(self.scene.palettes[palette_name])
        
        rgb_values = []
        for i, color_idx in enumerate(self.color):
            try:
                if isinstance(color_idx, int) and 0 <= color_idx < len(palette):
                    rgb_values.append(copy.deepcopy(palette[color_idx]))
                else:
                    rgb_values.append([255, 0, 0])
                    
                import logging
                logger = logging.getLogger("color_signal_system")
                logger.debug(f"Segment {self.segment_ID} color {i}: index={color_idx}, RGB={rgb_values[-1]}")
        ...
        while len(rgb_values) < 4:
            if rgb_values:
                rgb_values.append(copy.deepcopy(rgb_values[-1]))
            else:
                rgb_values.append([255, 0, 0])
    ...
    def from_dict(cls, data):
        ...
        segment.current_position = float(data["initial_position"])
