...
class LEDSimulator:    
    def __init__(self, scene_manager: SceneManager = None, scene: LightScene = None):
        """
        Initializes the LED Simulator with pygame
        """
        pygame.init()
        pygame.font.init()
        ...
        self.active_effect_id = self.scene.current_effect_ID if self.scene.current_effect_ID else min(self.scene.effects.keys())
    ...
    def _build_top_panel_one_row(self):
        """
        Build the top panel in one row
        """
        scale = self.ui_state['scale_factor']
        row_y = int(10 * scale)
        ...
        self.ui_elements['save_button'] = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect(current_x, row_y, int(90 * scale), button_height),
            text='Save Json',
            manager=self.manager,
            tool_tip_text="Save JSON"
        )
        current_x += int(100 * scale)
    ...
    def _handle_button_press(self, event):
        ...
        if event.ui_element == self.ui_elements.get('top_toggle'):
            ...
            self._show_file_dialog('save')
        
        elif event.ui_element == self.ui_elements.get('load_button'):
            self._show_file_dialog('load')
        ...
        elif event.ui_element == self.ui_elements.get('fade_toggle'):
            segment = self._get_active_segment()
            if segment:
                ...
                event.ui_element.set_text('ON' if segment.fade else 'OFF')
            ...
            if self.active_effect_id in self.scene.effects:
                ...
                self._add_notification(f"Segment {new_id} added")
            ...
            if self.active_effect_id in self.scene.effects:
                ...
                if self.active_segment_id in effect.segments:
                    ...
                    self.ui_dirty = True
                    self._add_notification(f"Segment {self.active_segment_id} removed")
            ...
            self.ui_dirty = True
            self._add_notification(f"Scene {new_scene_id} added")

        elif event.ui_element == self.ui_elements.get('remove_scene'):
            if self.scene_manager:
                if len(self.scene_manager.scenes) <= 1:
                    self._add_notification("Cannot remove the last scene")
                else:
                    ...
                    if self.scene_manager.scenes:
                        ...
                        self.active_scene_id = new_scene_id
                        self.ui_dirty = True
                        self._add_notification(f"Removed {scene_to_remove} and switched to scene {new_scene_id}")
                    else:
                        self.scene = None
                        self.active_scene_id = None
                        self.ui_dirty = True
                        self._add_notification(f"Scene {scene_to_remove} removed")
            else:
                self._add_notification("No scene manager available")
        ...
        if event.ui_element == self.ui_elements.get('scene_dropdown'):
            ...
            if self.scene_manager and scene_id in self.scene_manager.scenes:
                ...
                if self.scene.effects:
                    ...
                    if effect and effect.segments:
                        self.active_segment_id = min(effect.segments.keys())
                self.ui_dirty = True
                self._add_notification(f"Switched to scene {scene_id}")
            ...
            if effect_id in self.scene.effects:
                ...
                if effect.segments:
                    self.active_segment_id = min(effect.segments.keys())
                self.ui_dirty = True
                self._add_notification(f"Switched to effect {effect_id}")
            ...
            if palette_id in self.scene.palettes:
                ...
                self.ui_dirty = True
                self._add_notification(f"Switched from {old_palette} to {palette_id}")
    ...
    def _handle_event(self, event):
        ...
        if self.file_dialog_state['active']:
            return self._handle_file_dialog_event(event)
    ...
    def _handle_file_dialog_event(self, event):
        """
        Handle events in file dialog
        """
        if event.type == pygame.QUIT:
            return False
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                self.file_dialog_state['active'] = False
            elif event.key == pygame.K_RETURN:
                self._handle_file_dialog_ok()
            elif event.key == pygame.K_BACKSPACE:
                self.file_dialog_state['input_text'] = self.file_dialog_state['input_text'][:-1]
            else:
                if event.unicode.isprintable():
                    self.file_dialog_state['input_text'] += event.unicode
        
        elif event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                dialog_rects = self._draw_file_dialog()
                if dialog_rects and dialog_rects['dialog_rect'].collidepoint(event.pos):
                    self._handle_file_dialog_click(event.pos, dialog_rects)
                else:
                    self.file_dialog_state['active'] = False
    ...
    def _handle_key_press(self, event):
        ...
        if event.key == pygame.K_SPACE:
            ...
            if 'show_indicators' in self.ui_elements:
                self.ui_elements['show_indicators'].set_text(f"Guide: {'ON' if self.led_state['show_segment_indicators'] else 'OFF'}")
    ...
    def run(self):
        ...
        while running:
            ...
            if not self.file_dialog_state['active']:
    ...
    def _show_file_dialog(self, mode='save'):
        """
        Show custom file dialog using pygame
        """
        self.file_dialog_state['active'] = True
        self.file_dialog_state['mode'] = mode
        self.file_dialog_state['input_text'] = ''
    ...
    def _refresh_files_list(self):
        ...
        try:
            files = []
            for item in os.listdir(self.file_dialog_state['current_dir']):
                path = os.path.join(self.file_dialog_state['current_dir'], item)
                if os.path.isdir(path):
                    files.append(f"[DIR] {item}")
                elif item.endswith('.json'):
                    files.append(item)
            self.file_dialog_state['files_list'] = sorted(files)
        except Exception as e:
            self.file_dialog_state['files_list'] = []
    ...
    def _draw_file_dialog(self):
        ...
        dialog_width = 600
        dialog_height = 400
        dialog_x = (self.ui_state['width'] - dialog_width) // 2
        dialog_y = (self.ui_state['height'] - dialog_height) // 2
        
        dialog_surface = pygame.Surface((dialog_width, dialog_height), pygame.SRCALPHA)
        dialog_surface.fill((40, 40, 40, 230))
        pygame.draw.rect(dialog_surface, (200, 200, 200), (0, 0, dialog_width, dialog_height), 2)
        
        title = "Save File" if self.file_dialog_state['mode'] == 'save' else "Load File"
        title_text = self._render_text(title, 18, (255, 255, 255))
        dialog_surface.blit(title_text, (10, 10))
        
        dir_text = self._render_text(f"Dir: {self.file_dialog_state['current_dir']}", 12, (200, 200, 200))
        dialog_surface.blit(dir_text, (10, 40))
        
        files_y = 70
        for i, file_name in enumerate(self.file_dialog_state['files_list'][:15]):
            y_pos = files_y + i * 20
            color = (255, 255, 100) if file_name == self.file_dialog_state['selected_file'] else (255, 255, 255)
            file_text = self._render_text(file_name[:50], 12, color)
            dialog_surface.blit(file_text, (10, y_pos))
        
        input_y = dialog_height - 80
        pygame.draw.rect(dialog_surface, (60, 60, 60), (10, input_y, dialog_width - 20, 25))
        input_text = self._render_text(self.file_dialog_state['input_text'], 14, (255, 255, 255))
        dialog_surface.blit(input_text, (15, input_y + 3))
        
        button_y = dialog_height - 45
        ok_button = pygame.Rect(dialog_x + 10, dialog_y + button_y, 80, 30)
        cancel_button = pygame.Rect(dialog_x + 100, dialog_y + button_y, 80, 30)
        
        pygame.draw.rect(dialog_surface, (0, 120, 0), (10, button_y, 80, 30))
        pygame.draw.rect(dialog_surface, (120, 0, 0), (100, button_y, 80, 30))
        
        ok_text = self._render_text("OK", 14, (255, 255, 255))
        cancel_text = self._render_text("Cancel", 14, (255, 255, 255))
        dialog_surface.blit(ok_text, (35, button_y + 8))
        dialog_surface.blit(cancel_text, (115, button_y + 8))
        ...
        return {
            ...
            'files_area': pygame.Rect(dialog_x + 10, dialog_y + files_y, dialog_width - 20, 300),
            'input_area': pygame.Rect(dialog_x + 10, dialog_y + input_y, dialog_width - 20, 25)
        }
    
    def _handle_file_dialog_click(self, pos, dialog_rects):
        """
        Handle click in file dialog
        """
        if dialog_rects['ok_button'].collidepoint(pos):
            self._handle_file_dialog_ok()
        elif dialog_rects['cancel_button'].collidepoint(pos):
            self.file_dialog_state['active'] = False
        elif dialog_rects['files_area'].collidepoint(pos):
            files_y = dialog_rects['files_area'].y
            clicked_index = (pos[1] - files_y) // 20
            if 0 <= clicked_index < len(self.file_dialog_state['files_list']):
                selected = self.file_dialog_state['files_list'][clicked_index]
                if selected.startswith('[DIR]'):
                    dir_name = selected[6:]
                    if dir_name == '..':
                        self.file_dialog_state['current_dir'] = os.path.dirname(self.file_dialog_state['current_dir'])
                    else:
                        self.file_dialog_state['current_dir'] = os.path.join(self.file_dialog_state['current_dir'], dir_name)
                    self._refresh_files_list()
                else:
                    self.file_dialog_state['selected_file'] = selected
                    self.file_dialog_state['input_text'] = selected
    
    def _handle_file_dialog_ok(self):
        """
        Handle OK button press in file dialog
        """
        filename = self.file_dialog_state['input_text']
        if not filename:
            return
        ...
        full_path = os.path.join(self.file_dialog_state['current_dir'], filename)
        
        try:
            if self.file_dialog_state['mode'] == 'save':
                self._save_config_to_file(full_path)
            else:
                self._load_config_from_file(full_path)
            self.file_dialog_state['active'] = False
        except Exception as e:
            self._add_notification(f"Error: {str(e)}")
    ...
    def _save_config_to_file(self, filename):
        ...
        try:
            ...
            self._add_notification(f"Configuration saved: {filename}")
        except Exception as e:
    ...
    def _load_config_from_file(self, filename):
        ...
        try:
            ...
            self._add_notification(f"Configuration loaded: {filename}")
            self.ui_dirty = True
        except Exception as e:
            logger.error(f"Error loading config: {e}")
            self._add_notification(f"Error loading configuration: {e}")
    ...
    def _build_top_panel_two_rows(self):
        ...
        if remaining_width >= int(280 * scale):
            self.ui_elements['zoom_in'] = pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect(current_x, row1_y, int(35 * scale), button_height),
                text='+',
                manager=self.manager,
                tool_tip_text="Zoom In"
            )
            ...
            self.ui_elements['zoom_reset'] = pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect(current_x, row1_y, int(70 * scale), button_height),
                text='Reset',
                manager=self.manager,
                tool_tip_text="Restore Zoom and Pan"
            )
            ...
            self.ui_elements['show_indicators'] = pygame_gui.elements.UIButton(
                relative_rect=pygame.Rect(current_x, row1_y, int(110 * scale), button_height),
                text='Guide: ON' if self.led_state['show_segment_indicators'] else 'Guide: OFF',
                manager=self.manager,
                tool_tip_text="Show/Hide Segment Indicators"
            )
    ...
    def _draw_led_visualizer(self):
        """
        Draw visualizer for fade effect
        """
        if not self.fade_visualizer['show']:
            return
        ...
        current_time = int((segment.time * 1000) % cycle_time) if hasattr(segment, 'time') else 0
    ...
    def _update_auto_hide(self):
        """
        Update auto hide for UI panels
        """
    ...
    def _check_resizing_complete(self):
        """
        Check if resizing process is complete
        """
    ...
    def _draw_segment_indicators(self, display_rect, led_total_width, pan_offset):
        ...
        if self.scene_manager:
            total_leds = len(self.scene_manager.get_led_output())
        else:
            total_leds = len(self.scene.get_led_output())
        ...
        for segment_id, segment in effect.segments.items():
            ...
            move_range = segment.move_range if hasattr(segment, 'move_range') else [0, 0]
            
            min_range = min(move_range[0], move_range[1])
            max_range = max(move_range[0], move_range[1])
            
            segment_start_pos = min(segment_start_pos, total_leds - 1)
            
            indicator_start_x = display_rect.x + segment_start_pos * led_total_width + pan_offset
            
            max_possible_width = (total_leds - segment_start_pos) * led_total_width
            indicator_width = min(total_length_leds * led_total_width, max_possible_width)
            
            if indicator_start_x + indicator_width > display_rect.x and indicator_start_x < display_rect.right:
                
                visible_width = min(indicator_width, display_rect.right - indicator_start_x)
                visible_width = max(1, int(visible_width))
                
                alpha_surface = pygame.Surface((visible_width, display_rect.height), pygame.SRCALPHA)

                color = (255, 50, 50) if segment_id == self.active_segment_id else (50, 50, 255)
                color_with_alpha = (*color, self.led_state['segment_indicator_opacity'])
                alpha_surface.fill(color_with_alpha)
                
                dest_x = max(display_rect.x, int(indicator_start_x))
                self.screen.blit(alpha_surface, (dest_x, display_rect.y))
                ...
                if text_x + text_surface.get_width() > indicator_start_x + visible_width:
                    text_x = indicator_start_x + visible_width - text_surface.get_width() - 5
                
                text_x = max(display_rect.x, min(display_rect.right - text_surface.get_width() - 5, text_x))
                text_x = max(indicator_start_x, text_x)

                if text_x + text_surface.get_width() <= display_rect.right:
